...
class FormElementTwb extends ViewHelperFormElement
{
    ...
    public function render(ElementInterface $element, $formType = null, array $displayOptions = array())
    {
        ...
        if(in_array($type, array('select', 'textarea', 'text', 'password', 'url', 'date', 'datetime', 'fontPicker', 'email'))) {
            if(!isset($displayOptions['class'])) {
                $displayOptions['class'] = '';
            }
            $displayOptions['class'] = 'form-control '.$displayOptions['class'];
        }
        ...
        //FontPicker
        if ('fontPicker' == $type) {
            $helper = $renderer->plugin('form_font_picker_twb');
            return $helper($element, $formType, $displayOptions);
        }
        
        //Slider
        if ('slider' == $type) {
            $helper = $renderer->plugin('form_slider_twb');
            return $helper($element, $formType, $displayOptions);
        }
