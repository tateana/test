...
use DluTwBootstrap\Form\View\Helper\FormHelpTwb;
...
class FormRowTwb extends AbstractHelper
{
                           ...
                           $renderErrors = true)
    {
        ...
        $displayOptions['formType'] = $formType;
        ...
        if (($formType == FormUtil::FORM_TYPE_HORIZONTAL || $formType == FormUtil::FORM_TYPE_VERTICAL)
            ...
            $help                   = $helpHelper->render($element);
            if ($renderErrors) {
            ...
            if($formType == FormUtil::FORM_TYPE_HORIZONTAL) {
                $controlsOpen           = $controlsHelper->openTag($element, $formType);
                $controlsClose          = $controlsHelper->closeTag();
            }
        }
        
        if($element instanceof \Zend\Form\Element\Checkbox && !$element instanceof \Zend\Form\Element\MultiCheckbox) {
            $label = null;
        }
    ...
    /**
    * Retrieve the FormHelpTwb helper
    * @return FormHelpTwb
    * @throws \DluTwBootstrap\Form\Exception\UnsupportedHelperTypeException
    */
    protected function getHelpHelper()
    {
        if (!$this->helpHelper) {
            if (method_exists($this->view, 'plugin')) {
                $this->helpHelper = $this->view->plugin('form_help_twb');
            }
            if (!$this->helpHelper instanceof FormHelpTwb) {
                throw new UnsupportedHelperTypeException('Help helper (FormHelpTwb) unavailable or unsupported type.');
            }
        }
        return $this->helpHelper;
    }
